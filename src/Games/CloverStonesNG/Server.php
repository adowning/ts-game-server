...
{
    // Symbol mapping from numeric IDs to frontend symbol indices
    // Frontend expects indices that correspond to the symbol array positions
    private $symbolMap = [
        '0' => '0',  // Wild
        '1' => '1',  // TopAward
        '2' => '2',  // Group1
            ...
            switch ($reqId) {
                ...
                case 'AuthRequest':
                    ...
                    if ($requestHelper && method_exists($slotSettings, 'getAuthenticationDetails')) {
                        ...
                        // Generate initial symbols for the client (using numeric IDs)
                        $initialSymbols = [
                            ["8","8","10","9","7"],
                            ["10","10","7","6","8"],
                            ["9","9","9","7","10"],
                ...
                case 'SpinRequest':
                            ...
                            for ($k = 0; $k < $lines; $k++) { 
                                ...
                                foreach ($slotSettings->SymbolGame as $csym_idx => $csym) {
                                    ...
                                    for($r_idx=0; $r_idx<5; $r_idx++){
                                        ...
                                        // Convert line coordinate row position (0,1,2) to reel symbol index (1,2,3)
                                        // Our reels have 4 symbols [0,1,2,3] where [1,2,3] are the visible ones
                                        $symbolIndex = $lineCoordsDef[$r_idx][1] + 1;
                                        if(!isset($reelsForCascade['reel'.($r_idx+1)][$symbolIndex])) {
                                            $validSymbolPath = false; break;
                                        }
                                        $s[$r_idx] = $reelsForCascade['reel'.($r_idx+1)][$symbolIndex];
                                    }
                                    ...
                                    for ($reelIdx = 0; $reelIdx < 5; $reelIdx++) {
                                        if (isset($s[$reelIdx]) && ($s[$reelIdx] == $csym || $s[$reelIdx] == $wildSymbol)) {
                                            $winLength++;
                                            // Record the actual symbol position in the reel (1,2,3 for visible symbols)
                                            $actualSymbolsInWinPos[] = [$reelIdx, $lineCoordsDef[$reelIdx][1] + 1];
                                        } else {
                                            break;
                                        }
                                    }
                    ...
                    $spinResponseDetails = [
                        ...
                        "totalWin" => $totalWin, 
                        "winLines" => [], // Empty in serverResponse, actual win data is in data.slotWin
                        "reelsSymbols" => $reelsForResponse, 
                        "Jackpots" => $slotSettings->GetCurrentJackpotData(), 
                    ];

                    // Determine game state based on slot event and free spin status
                    $gameState = "Ready";
                    if ($slotEvent == 'freespin') {
                        // During free spins, state should be "FreeSpins"
                        $gameState = "FreeSpins";
                        // Check if free spins are completed
                        if ($currentFreeGames >= $totalFreeGames && $totalFreeGames > 0) {
                            $gameState = "Ready";
                        }
                    } else if ($slotEvent == 'bet' && $isBonusAwardedThisSpin) {
                        // When free spins are awarded during regular spin, stay in "Ready" state
                        // Let the frontend handle the transition to free spin mode via popup
                        $gameState = "Ready";
                    }

                    // Use original numeric symbols - frontend expects numeric IDs
                    $formattedRows = $this->formatReelsForResponse($reelsForResponse);
                    $finalResponseData = [
                        "state" => $gameState,
                        "spinResult" => [
                            "type" => "SpinResult",
                            "rows" => $formattedRows,
                            "columns" => $this->rowsToColumns($formattedRows) // Frontend expects columns format
                        ]
                    ];

                    // Add slotWin data if there are wins (matching original PHP format)
                    if ($totalWin > 0 && !empty($currentStageLineWins)) {
                        $finalResponseData["slotWin"] = [
                            "lineWinAmounts" => $currentStageLineWins,
                            "totalWin" => (string)$totalWin,
                            "canGamble" => "false"
                        ];
                    }

                    // Add bonus information when free spins are awarded during regular spin
                    if ($slotEvent == 'bet' && $isBonusAwardedThisSpin) {
                        // Add bonus win information to line wins
                        if (!isset($finalResponseData["slotWin"])) {
                            $finalResponseData["slotWin"] = [
                                "lineWinAmounts" => [],
                                "totalWin" => (string)$totalWin,
                                "canGamble" => "false"
                        ...
                        // Add bonus trigger information
                        $bonusInfo = [
                            "type" => "Bonus",
                            "bonusName" => "FreeSpins",
                            "params" => [
                                "freeSpins" => (string)$slotSettings->slotFreeCount
                            ],
                            "wonSymbols" => ""
                        ];

                        $finalResponseData["slotWin"]["lineWinAmounts"][] = $bonusInfo;

                        // Add bonusGameData that the frontend expects
                        $finalResponseData["bonusGameData"] = [
                            "newState" => "FreeSpins",
                            "freespinsCount" => $slotSettings->slotFreeCount
                        ];

                        // CRITICAL: Add freeSpinRemain that the frontend expects
                        $finalResponseData["freeSpinRemain"] = (string)$totalFreeGames;
                        $finalResponseData["freeSpinsTotal"] = (string)$totalFreeGames;

                        // CRITICAL: Add freeGame data structure that the popup expects
                        $finalResponseData["freeGame"] = [
                            "amount" => (string)$slotSettings->slotFreeCount,
                            "coin" => (string)($requestPayload['coin'] ?? 1),
                            "coinsPerBet" => (string)($requestPayload['bet'] ?? 1),
                            "lines" => (string)($requestPayload['lines'] ?? 20),
                            "freeGameExpiration" => date('Y-m-d H:i:s', time() + 86400) // 24 hours from now
                        ];

                        // CRITICAL: Add CloverStones-specific data for popup
                        $finalResponseData["freespinsCountForStartPopup"] = (string)$slotSettings->slotFreeCount;
                    }
                     if ($slotEvent == 'freespin') {
                        ...
                        if ($currentFreeGames >= $totalFreeGames && $totalFreeGames > 0) {
                             ...
                             // Add game parameters for returning to normal mode
                             $finalResponseData["gameParameters"] = [
                                 "initialSymbols" => $this->formatReelsForResponse($reelsForResponse)
                             ];
                        }
                    }

                    // Save game statistics for RTP calculation
                    $slotSettings->SaveLogReport($reelsForResponse, $betLine, $lines, $totalWin, $slotEvent);
    ...
    private function getLineCoords($lineIndex) {
        // Line coordinates for CloverStonesNG - 20 paylines
        // Each coordinate is [reel_index, row_position] where:
        // reel_index: 0-4 (for reels 1-5)
        // row_position: 0-2 (for the 3 visible symbols, since we use indices 1,2,3 from our 4-symbol reels)
        $lines = [
            [[0,1],[1,1],[2,1],[3,1],[4,1]], // Line 0: Middle row
            [[0,0],[1,0],[2,0],[3,0],[4,0]], // Line 1: Top row
            [[0,2],[1,2],[2,2],[3,2],[4,2]], // Line 2: Bottom row
            [[0,0],[1,1],[2,2],[3,1],[4,0]], // Line 3: V shape
            [[0,2],[1,1],[2,0],[3,1],[4,2]], // Line 4: ^ shape
            [[0,0],[1,0],[2,1],[3,2],[4,2]], // Line 5
            [[0,2],[1,2],[2,1],[3,0],[4,0]], // Line 6
            [[0,1],[1,0],[2,0],[3,0],[4,1]], // Line 7
            [[0,1],[1,2],[2,2],[3,2],[4,1]], // Line 8
            [[0,0],[1,1],[2,1],[3,1],[4,0]], // Line 9
            [[0,2],[1,1],[2,1],[3,1],[4,2]], // Line 10
            [[0,1],[1,0],[2,1],[3,2],[4,1]], // Line 11
            [[0,1],[1,2],[2,1],[3,0],[4,1]], // Line 12
            [[0,0],[1,1],[2,0],[3,1],[4,0]], // Line 13
            [[0,2],[1,1],[2,2],[3,1],[4,2]], // Line 14
            [[0,0],[1,0],[2,2],[3,0],[4,0]], // Line 15
            [[0,2],[1,2],[2,0],[3,2],[4,2]], // Line 16
            [[0,1],[1,0],[2,2],[3,0],[4,1]], // Line 17
            [[0,1],[1,2],[2,0],[3,2],[4,1]], // Line 18
            [[0,0],[1,1],[2,2],[3,1],[4,0]], // Line 19
        ];
    ...
    private function rowsToColumns($rows)
    {
        $columns = [];
        if (!empty($rows)) {
            $numReels = count($rows[0]); // Number of reels (columns)
            for ($reel = 0; $reel < $numReels; $reel++) {
                $column = [];
                for ($row = 0; $row < count($rows); $row++) {
                    if (isset($rows[$row][$reel])) {
                        $column[] = $rows[$row][$reel];
                    }
                }
                $columns[] = $column;
            }
    ...
    private function convertSymbolsToFrontendNames($reels)
    {
        $convertedReels = [];
        foreach ($reels as $reelKey => $reelData) {
            if ($reelKey === 'rp') {
                $convertedReels[$reelKey] = $reelData; // Keep rp as is
                continue;
            }

            $convertedReel = [];
            if (is_array($reelData)) {
                foreach ($reelData as $symbolId) {
                    $convertedReel[] = $this->symbolMap[$symbolId] ?? $symbolId;
                }
            }
            $convertedReels[$reelKey] = $convertedReel;
        }
