...
namespace VanguardLTE\Games\CloverStonesNG
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId)
        {
            ...
            $user = \VanguardLTE\Games\User::lockForUpdate()->find($this->playerId);
            $this->user = $user;
            $this->shop_id = $user->shop_id;
            $gamebank = \VanguardLTE\GameBank::where(['shop_id' => $this->shop_id])->lockForUpdate()->get();
            \Log::info($this->slotId);
            \Log::info($this->shop_id);
            $game = \VanguardLTE\Game::where([
                'name' => $this->slotId, 
                'shop_id' => $this->shop_id
            ])->lockForUpdate()->first();
            $this->shop = \VanguardLTE\Games\Shop::find($this->shop_id);
            $this->game = $game;
            ...
            $this->hideButtons = [];
            $this->jpgs = \VanguardLTE\Games\JPG::where('shop_id', $this->shop_id)->lockForUpdate()->get();
            $this->Line = [
                1, 
                2, 
                3, 
        ...
        {
            if( $this->game && $this->shop && $this->user && (!$this->game->view || $this->shop->is_blocked || $this->user->is_blocked || $this->user->status == \VanguardLTE\Games\Support\Enum\UserStatus::BANNED) ) 
            {
                \VanguardLTE\Games\Session::where('user_id', $this->user->id)->delete();
            ...
            if( $this->user->status == \VanguardLTE\Games\Support\Enum\UserStatus::BANNED ) 
            {
                return false;
            }
            return true;
            ...
            for( $i = 0; $i < count($this->jpgs); $i++ )
            {
                ...
                if( $this->jpgs[$i]->get_pay_sum() < $jsum[$i] && $this->jpgs[$i]->get_pay_sum() > 0 ) 
                {
                    ...
                    else
                    {
                        ...
                        {
                            \VanguardLTE\Games\StatGame::create([
        ...
        public function SaveLogReport($spinSymbols, $bet, $lines, $win, $slotState)
        {
            ...
            if( $gamebank ) 
            {
                list($slotsBank, $bonusBank, $fishBank, $tableBank, $littleBank) = \VanguardLTE\Games\Lib\Banker::get_all_banks($game->shop_id);
            }
            else
            {
