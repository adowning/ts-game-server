  ...
  //          {
    //            return redirect()->route('backend.shop.list')->withErrors(['You don\'t have limit']);
      //      }            
            ...
            $already =  \VanguardLTE\User::where('username', '=', $request->name)->exists();
            if(!$already ){
                ...
                $newUser->attachRole($roles->find(2));
...
//            \DB::statement('insert '.\DB::getTablePrefix().$g_table.'(name,title, shop_id, jpg_id,label, device,gamebank,chanceFirepot1,chanceFirepot2,chanceFirepot3,fireCount1,fireCount2,fireCount3,lines_percent_config_spin, lines_percent_config_spin_bonus lines_percent_config_bonus,lines_percent_config_bonus_bonusrezerv,cask,advanced,bet, scaleMode, slotViewState, view,denomination,category_temp, original_id, bids,stat_in, stat_out) (select name, title, '.$shop->id.' as shop_id, jpg_id, label, device,gamebank,chanceFirepot1,chanceFirepot2,chanceFirepot3,fireCount1,fireCount2,fireCount3,lines_percent_config_spin, lines_percent_config_spin_bonus lines_percent_config_bonus,lines_percent_config_bonus_bonusrezerv,cask,advanced,bet, scaleMode, slotViewState, view,denomination,category_temp, original_id, bids,stat_in, stat_out from '.\DB::getTablePrefix().$g_table.' where shop_id=1)');
