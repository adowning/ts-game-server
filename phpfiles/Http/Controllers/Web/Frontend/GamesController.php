...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function server(\Illuminate\Http\Request $request, $game)
        {
            ...
            //     exit();
            // }
            Log::info('Showing the user profile for user: {token}', ['token' => $request->userid]);
            Log::info('id', ['id' => $request->userid]);

            // $us = \VanguardLTE\User::where('api_token', '=', $request->token)->get();
            // Log::info($us);
            // if( isset($us[0]->id) )
            // {
            // Log::info('Showing the user profile for user: {id}', ['id' => $us[0]->id]);

                \Auth::loginUsingId($request->userid, true);

            Log::info('Auth user: {id}', ['id' => auth()->user()->id]);
            ...
            Log::info('subssession: {id}', ['id' => $subssession->id]);
